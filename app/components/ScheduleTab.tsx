...
export default function ScheduleTab({ slots: propSlots, teams: propTeams, divisions: propDivisions, params }: ScheduleTabProps) {
  ...
  // Smart conflict detection - finds same-day conflicts
  const detectConflicts = () => {
    ...
    console.log('🔍 Starting conflict detection on schedule with', currentSchedule.length, 'games')
    console.log('🔍 Sample games:', currentSchedule.slice(0, 3))
    ...
    // Build map of team -> dates they play
    currentSchedule.forEach((game, index) => {
      ...
      if (game.HomeTeam && game.AwayTeam) {
        ...
        // Track home team games
        if (!teamGameDates.has(homeTeam)) {
          teamGameDates.set(homeTeam, new Set())
        }
        teamGameDates.get(homeTeam)!.add(date)

        // Track away team games
        if (!teamGameDates.has(awayTeam)) {
          teamGameDates.set(awayTeam, new Set())
        }
        teamGameDates.get(awayTeam)!.add(date)
      }
    })

    // Find conflicts (teams playing multiple times on same date)
    console.log('🔍 Team game dates map:', Object.fromEntries(
      Array.from(teamGameDates.entries()).map(([team, dates]) => [
        team, 
        Array.from(dates)
      ])
    ))

    teamGameDates.forEach((dates, team) => {
      const dateCounts = new Map<string, number>()
      dates.forEach(date => {
        dateCounts.set(date, (dateCounts.get(date) || 0) + 1)
      })

      console.log(`🔍 Team ${team} plays on dates:`, Array.from(dates))
      console.log(`🔍 Date counts for ${team}:`, Object.fromEntries(dateCounts))

      dateCounts.forEach((count, date) => {
        if (count > 1) {
          console.log(`🚨 CONFLICT FOUND: ${team} plays ${count} times on ${date}`)
          
          // Conflict found - get all games for this team on this date
          const teamGames = currentSchedule
            .map((game, index) => ({ ...game, gameNumber: index + 1 }))
            .filter(game => {
              const gameHomeTeam = game.HomeTeam || game.home || game['Home Team']
              const gameAwayTeam = game.AwayTeam || game.away || game['Away Team']
              const gameDate = game.Date || game.date || game.start
              return (gameHomeTeam === team || gameAwayTeam === team) && gameDate === date
            })
            .map(game => ({
              gameNumber: game.gameNumber,
              home: game.HomeTeam || game.home || game['Home Team'] || '',
              away: game.AwayTeam || game.away || game['Away Team'] || '',
              time: game.Time || game.time || '',
              rink: game.Rink || game.rink || ''
            }))

          console.log(`🚨 Conflict games for ${team} on ${date}:`, teamGames)
    ...
    console.log('🔍 Final conflicts detected:', conflicts)
  ...
  // Smart conflict resolution using closest non-conflicting game swap
  const resolveConflicts = async () => {
    if (conflicts.length === 0) return

    setIsResolvingConflicts(true)
    setConflictResolutionResults(null)
    ...
    try {
      ...
      // Create a copy of the schedule to work with
      let workingSchedule = [...currentSchedule]
      
      for (const conflict of conflicts) {
        console.log(`🔄 Resolving conflict for ${conflict.team} on ${conflict.date}`)
        
        // Find the closest non-conflicting game we can swap with
        const swapResult = await findClosestNonConflictingSwap(workingSchedule, conflict)
        
        if (swapResult.success) {
          // Apply the swap
          workingSchedule = swapResult.newSchedule
          results.resolved++
          results.details.push(`✅ ${conflict.team}: Swapped with game ${swapResult.swappedGame.gameNumber} (${swapResult.swappedGame.date})`)
        } else {
          results.details.push(`❌ ${conflict.team}: No suitable swap found - ${swapResult.reason}`)
        }
      }

      // Sort the schedule chronologically after resolving conflicts
      workingSchedule = sortScheduleChronologically(workingSchedule)
      
      // Update the schedule with resolved conflicts
      setCurrentSchedule(workingSchedule)
      setConflictResolutionResults(results)
      
      // Re-detect conflicts to see what's left
  ...
  // Find the closest non-conflicting game to swap with
  const findClosestNonConflictingSwap = async (
    schedule: any[], 
    conflict: { team: string, date: string, games: Array<any> }
  ) => {
    const conflictTeam = conflict.team
    const conflictDate = conflict.date
    
    // Parse conflict date for comparison
    const conflictDateObj = new Date(conflictDate)
    
    // Find all games that don't involve the conflicting team
    const candidateGames = schedule
      .map((game, index) => ({ ...game, gameNumber: index + 1 }))
      .filter(game => 
        game.HomeTeam && game.AwayTeam && // Has teams assigned
        game.HomeTeam !== conflictTeam && 
        game.AwayTeam !== conflictTeam &&
        game.Date !== conflictDate // Not on the same date
      )

    if (candidateGames.length === 0) {
      return { success: false, reason: 'No candidate games found' }
    }
    ...
    candidateGames.sort((a, b) => {
      ...
      const diffA = Math.abs(dateA.getTime() - conflictDateObj.getTime())
      const diffB = Math.abs(dateB.getTime() - conflictDateObj.getTime())
      return diffA - diffB
    })

    // Try each candidate game for a valid swap
    for (const candidate of candidateGames) {
      const swapResult = await testSwap(schedule, conflict, candidate)
      if (swapResult.success) {
        return {
          success: true,
          swappedGame: candidate,
          newSchedule: swapResult.newSchedule
        }
      }
    }

    return { success: false, reason: 'No valid swaps found' }
  }

  // Test if a swap would work without creating new conflicts
  const testSwap = async (
    schedule: any[], 
    conflict: { team: string, date: string, games: Array<any> },
    candidateGame: any
  ) => {
    // Create a copy of the schedule for testing
    const testSchedule = [...schedule]
    
    // Get the first conflict game (we'll swap this one)
    const conflictGame = conflict.games[0]
    const conflictGameIndex = conflictGame.gameNumber - 1
    
    // Get the candidate game index
    const candidateGameIndex = candidateGame.gameNumber - 1
    
    // Test the swap
    const tempHome = testSchedule[conflictGameIndex].HomeTeam
    const tempAway = testSchedule[conflictGameIndex].AwayTeam
    
    // Swap the teams
    testSchedule[conflictGameIndex].HomeTeam = testSchedule[candidateGameIndex].HomeTeam
    testSchedule[conflictGameIndex].AwayTeam = testSchedule[candidateGameIndex].AwayTeam
    testSchedule[candidateGameIndex].HomeTeam = tempHome
    testSchedule[candidateGameIndex].AwayTeam = tempAway
    
    // Check if this creates new conflicts
    const newConflicts = detectConflictsInSchedule(testSchedule)
    const hasNewConflicts = newConflicts.some(c => 
      c.team === conflict.team || 
      c.team === testSchedule[candidateGameIndex].HomeTeam ||
      c.team === testSchedule[candidateGameIndex].AwayTeam
    )
    
    if (!hasNewConflicts) {
      return { success: true, newSchedule: testSchedule }
    }
    
    return { success: false, reason: 'Swap creates new conflicts' }
  }

  // Helper function to detect conflicts in a schedule
  const detectConflictsInSchedule = (schedule: any[]) => {
    const teamGameDates = new Map<string, Set<string>>()
    const conflicts: Array<{ team: string, date: string }> = []

    schedule.forEach(game => {
      // Check for different possible field names (same logic as main function)
      const homeTeam = game.HomeTeam || game.home || game['Home Team']
      const awayTeam = game.AwayTeam || game.away || game['Away Team']
      const date = game.Date || game.date || game.start
                  ...
                  {conflicts.map((conflict, index) => (
                    <div key={index} className="bg-red-800/30 rounded p-3">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-red-200 font-medium">{conflict.team}</span>
                        <span className="text-red-400">plays {conflict.games.length} times on</span>
                        <span className="text-red-200 font-medium">{conflict.date}</span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        {conflict.games.map((game, gameIndex) => (
                          <div key={gameIndex} className="bg-red-700/30 rounded p-2">
                            <div className="text-red-300 font-medium">Game #{game.gameNumber}</div>
                            <div className="text-red-400">{game.home} vs {game.away}</div>
                            <div className="text-red-400">{game.time} - {game.rink}</div>
                          </div>
                        ))}
                      </div>
                    </div>
